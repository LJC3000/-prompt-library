...
interface PromptModalProps {
  ...
  selectedImageToken: string | null;
  hasNext: boolean;
  hasPrev: boolean;
  onNext: () => void;
  onPrev: () => void;
...
export default function PromptModal({ prompt, selectedCardKey, selectedImageToken, hasNext, hasPrev, onNext, onPrev, onClose, preloadedUrls }: PromptModalProps) {
  ...
  return (
    <>
      <AnimatePresence>
        {prompt && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center">
            {/* Overlay — independent opacity-only, no layoutId */}
            <motion.div
              key="modal-overlay"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            {/* Modal body — shared element target */}
            <motion.div
              key="modal-body"
              layoutId={selectedCardKey ? `card-${selectedCardKey}` : undefined}
              className={`relative z-10 mx-4 bg-white rounded-2xl shadow-2xl overflow-hidden ${isTall ? "flex h-[80vh]" : ""}`}
              style={{ maxWidth: isTall ? undefined : "48rem" }}
            ...
            >
              ...
              {!isTall && prompt.results?.[0] && (
                ...
                >
                  <motion.img
                    layoutId={selectedImageToken ? `image-${selectedImageToken}` : undefined}
                    src={mainImgSrc ?? undefined}
                    alt={prompt.title}
                    ...
                    onClick={() => mainImgSrc && setViewerSrc(mainImgSrc)}
                  />
                  {mainImgSrc && (
              ...
              {/* === Drawer: text content — delayed slide-in from right === */}
              <motion.div
                initial={{ opacity: 0, x: 50 }}
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: 20, transition: { duration: 0.15 } }}
                transition={{ delay: 0.15, type: "spring", stiffness: 200, damping: 20 }}
                className={isTall ? "flex flex-col w-[380px] shrink-0 h-full" : ""}
              >
                {/* Non-tall layout: ref images (left) + details (right) */}
                {!isTall && (
                ...
                {/* Tall layout: right column */}
                {isTall && (
                  <div className="flex flex-col w-[380px] shrink-0 h-full overflow-y-auto">
                    {/* Title + copy + category */}
      ...
      {/* Nav buttons — outside AnimatePresence, position-independent */}
      {prompt && <NavButton side="left" onClick={onPrev} disabled={!hasPrev} />}
      {prompt && <NavButton side="right" onClick={onNext} disabled={!hasNext} />}

      {/* Image viewer — own AnimatePresence for independent enter/exit */}
      <AnimatePresence>
